...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        sentence_padded = sentence + [zero_vector] * (max_length - len(sentence))
        labels_padded = labels + [zero_label] * (max_length - len(sentence))
        mask = [True] * len(labels) + [False] * (max_length - len(sentence))
        ret.append((sentence_padded[:max_length], labels_padded[:max_length], mask[:max_length]))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        elif self.config.cell == "sru":
            cell = SRUCell(Config.n_features * Config.embed_size, Config.hidden_size)
        else:
...
if __name__ == "__main__":
    ...
    command_parser.add_argument('-c', '--cell', choices=["rnn", "gru", "sru"], default="rnn", help="Type of RNN cell to use.")
